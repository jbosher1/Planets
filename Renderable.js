 ...
 * CoordinateSystem inherits from CoordinateRenderable.  CoordinateSystem represents a 2d coordinate system
   * It has :"" objects of type CoordinateSystem and type Shape.
 *
...
{
    return this.parent_from_local().inverse();
};
...
CoordinateSystem.prototype.local_from_world = function ()
{
  if (this.parent == null) {
    ...
    parent = this.parent.local_from_world();
    console.log(this.parent);
    return this.local_from_parent().multiply(parent);
  }
};
...
{
    //this.parent_from_local().multiply(this.local_from_parent())
    modelViewStack.push();
...
CoordinateSystem.prototype.update = function(change, velocity){
  if (this.velocity != 0){
    this.orientation = (this.orientation + (velocity * this.velocity * change)/ 1000.0) % 360;
  }

  for (var i = 0; i < this.child.length; i++){
    this.child[i].update(change, velocity);
  }
}
...
var UnitDisc = function (gl, shader, info) {
  Shape.call(this);

  this.name = info.name;
  this.selectable = info.selectable;
  ...
  for (var i = 0; i <= this.numVertices; i++) {
    ...
    this.renderable.vertices.push([this.center.x + this.radius * Math.sin(angle), this.center.y + this.radius * Math.cos(angle)]);
  }

  this.renderable.updateBuffers();
  this.renderable.color.set(info.color);
};
...
UnitDisc.prototype.point_inside = function (point_world) {
  // compute point coordinate in local coordinate system
  var local_point = new Vec3();
  local_point.x = point_world.x;
  local_point.y = point_world.y;
  local_point.w = 1.0;
  if (this.parent !== null) {
    local_point = local_point.multiply(this.parent.local_from_world());
  }

  // perform containment test in local coordinate space
  //console.log("  lcs: " + point_lcs.x + ", " + point_lcs.y);
  return (Math.sqrt(Math.pow(local_point.x - this.center.x, 2) + Math.pow(local_point.y - this.center.y, 2)) <= Math.pow(this.radius, 2));
};
